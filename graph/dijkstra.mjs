  ...
  shortestPath(start_city, end_city){
    ...
    while(Object.keys(unvisited_cities).length > 0){
      ...
      for(let adjacent_city_name in visited_cities[closest_city_name].adjacent_cities){
        ...
        let distance = shortest_path_table[closest_city_name].distance + // 출발도시에서 현재 도시까지의 거리
    ...
    // 최단 경로 테이블을 이용해서 출발 도시에서 도착 도시까지 가는 최단 경로를 문자열로 만들어서 출력
    
  }
  showShortestPathRecursively(destination_city_name, shortest_path_table, path_string = ""){
    // 매개변수로는 최단 경로를 구할 도착 도시의 이름인 destination_city_name과 최단 경로 테이블인 shortest_path_table, 재귀적으로 호출하기 때문에 각 함수에서 저장한 경로를 전달해줄 path_string
    if(shortest_path_table[destination_city_name].city == null){
      // 기저조건은 shortest_path_table에서 목적지 도시의 이전 방문 도시가 없는 경우
      // 이때는 path_string에 목적지 도시의 이름을 추가
    ...
    // 그렇지 않다면 목적지 도시의 이름을 path_string에 추가하고 이전 방문 도시를 대상으로 재귀적으로 호출
    // 다만 목적지 도시를 현재 도시의 이전 방문 도시로 바꿔서 호출  
